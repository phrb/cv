...
\geometry{
    hmargin=1.33cm, % Horizontal margin
    vmargin=1.1cm, % Vertical margin
...
\setlength\columnsep{0.06\textwidth} % Specify the amount of space between the columns
...
%\newfontfamily{\FA}{Font Awesome 5 Brands} % Create a new font family for FontAwesome icons, provided with the template in the fonts folder
...
\definecolor{text}{HTML}{000000} % Main document font colour, off-black
\definecolor{headings}{HTML}{165075} % Dark red colour for headings
\definecolor{shade}{HTML}{ebebeb} % Peach colour for the contact information box
\definecolor{linkcolor}{HTML}{165075} % 25% desaturated headings colour for links
...
\titleformat{\section}{\color{headings}\bf\large\raggedright}{}{0em}{} % Define the \section format
...
\newcommand{\workposition}[5]{
    {\small\textsc{#1}} & \textbf{#2}\\ % Duration and Job Title
    \expandafter\ifstrequal\expandafter{#3}{}{}{& {\small\textit{#3}}\\} % Institution / Lab
    \expandafter\ifstrequal\expandafter{#4}{}{}{& {\small\textit{#4}}\\[4pt]} % University
    \expandafter\ifstrequal\expandafter{#5}{}{}{& {\small#5}\\[4pt]} % Description
}
...
\newcommand{\tableheaderthree}[4]{
    \textbf{#1} & \textbf{#2} & \textbf{#3}\expandafter\ifstrequal\expandafter{#4}{}{\\}{\\[6pt]} % First the heading, then content, then a conditional insertion of whitespace if the third parameter has any content in it
}
\newcommand{\tableheadertwo}[3]{
    \textbf{#1} & \textbf{#2}\expandafter\ifstrequal\expandafter{#3}{}{\\}{\\[6pt]} % First the heading, then content, then a conditional insertion of whitespace if the third parameter has any content in it
}
\newcommand{\tableentrythree}[4]{
    \textsc{#1} & #2 & #3\expandafter\ifstrequal\expandafter{#4}{}{\\}{\\[6pt]} % First the heading, then content, then a conditional insertion of whitespace if the third parameter has any content in it
}
\newcommand{\tableentrythreeskills}[4]{
    {\small#1} & {\small#2} & {\small#3}\expandafter\ifstrequal\expandafter{#4}{}{\\}{\\[6pt]} % First the heading, then content, then a conditional insertion of whitespace if the third parameter has any content in it
}
\newcommand{\tableentrytwoskills}[3]{
    #1 & #2\expandafter\ifstrequal\expandafter{#3}{}{\\[6pt]}{\\[6pt]} % First the heading, then content, then a conditional insertion of whitespace if the third parameter has any content in it
}
